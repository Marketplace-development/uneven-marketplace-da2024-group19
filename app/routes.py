...
@main.route('/demand_index', methods=['GET'])
def calculate_demand_index():
    """
    Calculate the demand index for each province based on the number of available dog sitters.
    """
    # Fetch all listings grouped by location (province)
    provinces = db.session.query(Listing.location, db.func.count(Listing.id)).group_by(Listing.location).all()

    # Determine max and min sitter counts for normalization
    sitter_counts = [count for _, count in provinces]
    max_sitters = max(sitter_counts) if sitter_counts else 1  # Avoid division by zero
    min_sitters = min(sitter_counts) if sitter_counts else 0

    # Calculate demand index for each province
    demand_index = {}
    for location, count in provinces:
        # Normalize: Invert the sitter count (fewer sitters = higher demand)
        demand_index[location] = round((max_sitters - count) / max_sitters, 2)  # Scaled between 0 and 1 and round two digits after 'komma'
